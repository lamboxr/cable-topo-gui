...
def show_current_settings():
    ...
    # 显示保存目录
    last_dir = settings.value("last_save_directory")
    if last_dir:
        print(f"上次保存目录: {last_dir}")
        print(f"目录是否存在: {'是' if os.path.exists(last_dir) else '否'}")
    else:
        print("上次保存目录: 未设置")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  python config_manager.py set <目录>      # 设置默认保存目录")
        print("\n示例:")
        print("  python config_manager.py show")
        print("  python config_manager.py set C:\\Users\\用户名\\Documents")
        return
